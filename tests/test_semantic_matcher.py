...
class TestSemanticDataMatcher:
    ...
    def matcher(self):
        ...
        return SemanticDataMatcher(
            model_name="all-MiniLM-L6-v2", similarity_threshold=0.5, max_matches=10
        )
    ...
    def test_match_multiple_data_sources(self, matcher):
        ...
        matcher.register_data("proj1", "api_authentication", {"method": "JWT"})
        matcher.register_data(
            "proj1", "database_schema", {"tables": ["users", "posts"]}
        )
    ...
    def test_match_respects_threshold(self, matcher):
        ...
        matcher.register_data(
            "proj1", "weather_data", {"temperature": 72, "conditions": "sunny"}
        )

        # Match with completely different topic
        matches = matcher.match_agent_needs(
            "proj1", ["database schema and table structure"]
        )
    ...
    def test_match_respects_max_matches(self):
        ...
        for i in range(10):
            matcher.register_data(
                "proj1", f"api_endpoint_{i}", {"endpoint": f"/api/v1/resource{i}"}
            )

        # Match should return at most 2 results
        matches = matcher.match_agent_needs("proj1", ["API endpoints"])
    ...
    def test_match_sorts_by_similarity(self, matcher):
        ...
        matcher.register_data("proj1", "related", {"security": "SSL"})
        matcher.register_data("proj1", "unrelated", {"logging": "enabled"})

        matches = matcher.match_agent_needs(
            "proj1", ["authentication and authorization"]
        )
        ...
        for need_matches in matches.values():
            if len(need_matches) > 1:
                for i in range(len(need_matches) - 1):
                    assert (
                        need_matches[i]["similarity"]
                        >= need_matches[i + 1]["similarity"]
                    )
