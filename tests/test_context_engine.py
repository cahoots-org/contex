...
class TestContextEngine:
    ...
    async def test_register_agent_matches_existing_data(self, context_engine):
        ...
        registration = AgentRegistration(
            agent_id="agent1",
            project_id="proj1",
            data_needs=["authentication methods"]
        )
    ...
    async def test_unregister_agent(self, context_engine):
        ...
        # Register first
        await context_engine.register_agent(AgentRegistration(
            agent_id="agent1",
            project_id="proj1",
            data_needs=["data"]
        ))
    ...
    async def test_current_sequence_tracking(self, context_engine):
        ...
        # Register agent
        response = await context_engine.register_agent(AgentRegistration(
            agent_id="agent1",
            project_id="proj1",
            data_needs=["data"]
        ))
...
class TestContextSizeLimits:
    ...
    async def test_context_size_limit_applied(self, context_engine_with_limit):
        ...
        # Publish large data
        for i in range(5):
            await context_engine_with_limit.publish_data(DataPublishEvent(
                project_id="proj1",
    ...
    async def test_truncation_keeps_best_matches(self, context_engine_with_limit):
        ...
        await context_engine_with_limit.publish_data(DataPublishEvent(
            ...
            data={"content": "general programming tips " * 100}  # Large but less relevant
        ))
