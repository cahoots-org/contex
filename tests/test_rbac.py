...
class TestRoleAssignment:
    ...
    @pytest.mark.asyncio
    async def test_get_role(self, db):
        """Test retrieving a role assignment"""
        await create_api_key(db, "test_key_3")
        # Assign a role first
        await assign_role(db, "test_key_3", Role.CONSUMER, ["proj1"])

        # Retrieve it
        role_assignment = await get_role(db, "test_key_3")
    ...
    @pytest.mark.asyncio
    async def test_get_role_default_readonly(self, db):
        """Test that non-existent roles default to readonly"""
        role_assignment = await get_role(db, "nonexistent_key")

        assert role_assignment.role == Role.READONLY
        assert role_assignment.projects == []

    @pytest.mark.asyncio
    async def test_revoke_role(self, db):
        """Test revoking a role assignment"""
        await create_api_key(db, "test_key_4")
        # Assign a role
        await assign_role(db, "test_key_4", Role.PUBLISHER, ["proj1"])

        # Revoke it
        success = await revoke_role(db, "test_key_4")
        assert success

        # Should now default to readonly
        role_assignment = await get_role(db, "test_key_4")
        assert role_assignment.role == Role.READONLY

    @pytest.mark.asyncio
    async def test_revoke_nonexistent_role(self, db):
        """Test revoking a non-existent role"""
        success = await revoke_role(db, "nonexistent_key")
        assert not success
    ...
    async def test_list_roles(self, db):
        ...
        await create_api_key(db, "key1")
        await create_api_key(db, "key2")
        await create_api_key(db, "key3")
        # Assign multiple roles
        await assign_role(db, "key1", Role.ADMIN, [])
        await assign_role(db, "key2", Role.PUBLISHER, ["proj1"])
        await assign_role(db, "key3", Role.CONSUMER, ["proj2", "proj3"])
