...
class WebhookDispatcher:
    ...
    async def send_webhook(
        ...
        for attempt in range(self.max_retries):
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    print(f"[WebhookDispatcher] ⚠ Webhook returned {response.status_code}: {url}")
                ...
                print(f"[WebhookDispatcher] ⚠ Timeout on attempt {attempt + 1}/{self.max_retries}")
        ...
        print(f"[WebhookDispatcher] ✗ Failed to deliver webhook after {self.max_retries} attempts")
        return False
