...
class EventStore:
    ...
    async def append_event(
        ...
        print(f"[EventStore] Appended event: {project_id}:{event_type} (seq: {sequence})")
    ...
    async def get_events_since(
        ...
        if results:
            for stream_name, stream_events in results:
                for event_id, event_data in stream_events:
                    ...
                    event_type_bytes = event_data.get(b"event_type") or event_data.get("event_type")
                    event_type = event_type_bytes.decode() if isinstance(event_type_bytes, bytes) else event_type_bytes
        ...
        print(f"[EventStore] Retrieved {len(events)} events since {since_id} for {project_id}")
