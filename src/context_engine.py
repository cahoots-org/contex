...
class ContextEngine:
    ...
    def __init__(
        ...
        try:
            ...
            self.tokenizer = None
            print("[ContextEngine] ⚠ Tiktoken unavailable, context size limits disabled")
    ...
    def _truncate_matches(
        ...
        print(f"[ContextEngine] ⚠ Context size ({total_tokens} tokens) exceeds limit ({max_tokens} tokens)")
        ...
        print(f"[ContextEngine]   Kept {truncated_count}/{original_count} matches ({final_tokens} tokens)")
    ...
    async def register_agent(self, registration: AgentRegistration) -> RegistrationResponse:
        """
        ...
        if registration.notification_method == "webhook":
            if not registration.webhook_url:
                raise ValueError("webhook_url is required when notification_method='webhook'")

        # 4. Determine notification channel (for Redis mode)
        notification_channel = registration.notification_channel or f"agent:{agent_id}:updates"
        ...
        self.agents[agent_id] = {
            ...
            "webhook_url": str(registration.webhook_url) if registration.webhook_url else None,
        ...
        # 10. Build response
        matched_counts = {need: len(need_matches) for need, need_matches in matches.items()}
    ...
    async def _send_initial_context(
        ...
        if registration.notification_method == "redis":
            ...
            if success:
                ...
                print(f"[ContextEngine] ⚠ Failed to send initial context to {agent_id} via webhook")
    ...
    async def _notify_affected_agents(
        ...
        print(f"[ContextEngine] Notifying {len(affected_agents)} agents about {data_key} update")
    ...
    def query_project_data(
        ...
        try:
            ...
            matches = self.semantic_matcher.match_agent_needs(
                project_id,
                [query]
            )
