...
class SemanticDataMatcher:
    ...
    def __init__(
        ...
        print(f"[SemanticMatcher] ✓ Model loaded (threshold: {similarity_threshold}, max_matches: {max_matches})")
        ...
        for need in needs:
            ...
            for registry_key, info in self.registry.items():
                ...
                similarity = cosine_similarity(
                    need_embedding.reshape(1, -1),
                    info["embedding"].reshape(1, -1)
                )[0][0]
            ...
            if top_matches:
                ...
                for match in top_matches:
                    print(f"[SemanticMatcher]     - {match['data_key']} (similarity: {match['similarity']:.3f})")
            else:
                print(f"[SemanticMatcher]   ⚠ No matches found (threshold: {self.threshold})")
    ...
    def clear_project(self, project_id: str):
        ...
        to_remove = [
            key for key in self.registry.keys()
            if key.startswith(f"{project_id}:")
        ]
        ...
        print(f"[SemanticMatcher] Cleared {len(to_remove)} entries for project {project_id}")
